...
#setwd('C:/Users/Home/Desktop/Compu Lab/Tutorial')
...
simpleAssignToCentroids <- function(points = testPoints, centroids=testCentroids)
{
  ...
  sqErrrorMat = matrix(0 , nrow =numberOfPoints ,ncol =numberOfCentroids)
  ...
  for (pointNo in 1:numberOfPoints)
  {
    ...
    for (centroidNo in 1:numberOfCentroids)
    {
      ...
      sqErrrorMat[pointNo, centroidNo] <- (points[pointNo,1] - centroids[centroidNo,1])^2+(points[pointNo,2] - centroids[centroidNo,2])^2
      ...
      sqErrrorMat[is.na(sqErrrorMat)] <- Inf
  ...
  return(apply(sqErrrorMat,1, function(x){which(x == min(x))}))
